...
template<typename E> class AList :public List<E> {			// Array-based list implementataion(й╣ож)
private:
...
public:
	AList(int size = defaultSize) {  //Constructor, user set the maxSize
		maxSize = size;
		listSize = curr = 0;
	...
	void clear() {			// Reinitialize the list
		...
		listSize = curr = 0;	// Reset the size and the current element
